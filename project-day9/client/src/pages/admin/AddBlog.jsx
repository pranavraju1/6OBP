...
const AddBlog = () => {
  ...
  const [isPublished, seIspublished] = useState(false);
    ...
    <form onSubmit={onSubmitHandler} className="flex-1 bg-blue-50/50 text-gray-600 h-full overflow-scroll">
      <div className="bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow rounded">
        ...
        <input 
          ...
          className="w-full max-w-lg mt-2 p-2 borderborder-gray-300 outline-none rounded" 
        ...
        <div className="flex gap-2 mt-4">
          ...
          <input type="checkbox" checked={isPublished} className="scale-125 cursor-pointer"
            onChange={e=>seIspublished(e.target.checked)}
          />
        </div>

        <button disabled={isAdding} type="submit" className="mt-8 w-40 h-10 bg-primary text-white rounded
